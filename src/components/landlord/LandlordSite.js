...
function SiteSlideshow({ slides }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', aspectRatio: '16/6', overflow: 'hidden', background: '#3A5740', minHeight: 200 }}>
      ...
      />
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(0,0,0,0.02) 0%, rgba(0,0,0,0.12) 100%)' }} />
      {slides.length > 1 && (
    ...
    <>
      ...
      <footer style={{ background: 'var(--charcoal)', color: 'rgba(255,255,255,0.7)', padding: '28px 20px', textAlign: 'center', fontSize: 13 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: '6px 18px' }}>
          ...
          <Link href="/terms" style={{ color: 'var(--sage-light)', textDecoration: 'none', fontSize: 13 }}>免責聲明</Link>
        </div>
      </footer>
    </>
  )
